...
# Build index with higher pruning threshold (150)
...
for top_k in [15, 20, 25]:
    ...
    for sid in val_s1_ids:
        ...
        cands = [tid for tid, _ in counts.most_common(top_k)]
        retrieved += len(val_gt[sid] & set(cands))
        total_cands += len(cands)
    recall = retrieved / total_true
    print(f'Top-K={top_k}: Candidate Recall={retrieved}/{total_true} = {recall*100:.2f}%, Avg Cands={total_cands/len(val_s1_ids):.2f}')
